...
async function runReleaseGate(){
 ...
 const box=q("releaseGateResult");if(box){box.className="health "+(score===100?"ok":score>=80?"warn":"bad");box.innerHTML=`V20.8 Release Gate：${score}%｜${pass}/${checks.length} PASS｜狀態：${status}<ul class="auditList">${checks.map(i=>`<li>${i.ok?"✅":"⚠️"} ${i.name}${i.detail?"｜"+esc(i.detail):""}</li>`).join("")}</ul>`}
...
function downloadRegistry(){dl(new Blob([JSON.stringify(BUTTON_ACTION_REGISTRY,null,2)],{type:"application/json;charset=utf-8"}),"BUTTON_ACTION_REGISTRY_V20.8.json")}
function downloadE2EMatrix(){dl(new Blob([JSON.stringify(E2E_TEST_MATRIX,null,2)],{type:"application/json;charset=utf-8"}),"E2E_TEST_MATRIX_V20.8.json")}
...
async function resetMockFixtures(){
 ...
 if(!cur){
  const p={id:"TEST-"+Date.now(),name:"V20.8 Mock Project",client:"E2E",address:"Fixture",sales:"",eng:"",type:"LED",note:"",status:"測試",m:Date.now(),data:{o:[],scenes:[{id:"S1",name:"Fixture Scene",objects:[],bgScene:{w:1280,h:720},bgTransform:{rotation:0,scale:1}}],a:[],v:[],groups:{}}};
...
function exportMockE2E(){
 ...
 dl(new Blob([JSON.stringify(r,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8_Mock_E2E.json")
}
...
function exportAgentLog(){
 ...
 dl(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8_Agent_Log.json")
}
...
function exportEngineeringOptimization(){
 const r=engineeringOptimization||optimizeSelectedEngineering();if(!r)return;
 dl(new Blob([JSON.stringify(r,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8_Engineering_Optimization.json")
}
