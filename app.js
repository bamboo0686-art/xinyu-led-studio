...
async function runReleaseGate(){
 ...
 const box=q("releaseGateResult");if(box){box.className="health "+(score===100?"ok":score>=80?"warn":"bad");box.innerHTML=`V20.8.1 Release Gate：${score}%｜${pass}/${checks.length} PASS｜狀態：${status}<ul class="auditList">${checks.map(i=>`<li>${i.ok?"✅":"⚠️"} ${i.name}${i.detail?"｜"+esc(i.detail):""}</li>`).join("")}</ul>`}
...
q("new").onclick=()=>q("modal").classList.add("show");q("cancel").onclick=()=>q("modal").classList.remove("show");q("create").onclick=async()=>{
 const p={id:"P"+Date.now(),name:q("nn").value||"新LED專案",client:q("nc").value,address:q("na").value,sales:q("ns").value,eng:q("ne").value,type:q("nt").value,note:q("nno").value,status:"草稿",m:Date.now(),data:{o:[],scenes:[{id:"S1",name:"場景1",objects:[],bgScene:{w:0,h:0},bgTransform:{rotation:0,scale:1}}],a:[],v:[],groups:{}}};
...
async function open(id){
 ...
 try{
  ...
  if(!Array.isArray(scenes)||!scenes.length)scenes=[{id:"S1",name:"場景1",objects:[],bgScene:{w:0,h:0}}];
...
/* === V20.8.1 屬性整合＋多模型＋裸眼3D＋媒體生命週期修正版 === */
...
function downloadRegistry(){dl(new Blob([JSON.stringify(BUTTON_ACTION_REGISTRY,null,2)],{type:"application/json;charset=utf-8"}),"BUTTON_ACTION_REGISTRY_V20.8.1.json")}
function downloadE2EMatrix(){dl(new Blob([JSON.stringify(E2E_TEST_MATRIX,null,2)],{type:"application/json;charset=utf-8"}),"E2E_TEST_MATRIX_V20.8.1.json")}
...
async function resetMockFixtures(){
 ...
 if(!cur){
  const p={id:"TEST-"+Date.now(),name:"V20.8.1 Mock Project",client:"E2E",address:"Fixture",sales:"",eng:"",type:"LED",note:"",status:"測試",m:Date.now(),data:{o:[],scenes:[{id:"S1",name:"Fixture Scene",objects:[],bgScene:{w:1280,h:720},bgTransform:{rotation:0,scale:1}}],a:[],v:[],groups:{}}};
...
function exportMockE2E(){
 ...
 dl(new Blob([JSON.stringify(r,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8.1_Mock_E2E.json")
}
...
function exportAgentLog(){
 ...
 dl(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8.1_Agent_Log.json")
}
...
function exportEngineeringOptimization(){
 const r=engineeringOptimization||optimizeSelectedEngineering();if(!r)return;
 dl(new Blob([JSON.stringify(r,null,2)],{type:"application/json"}),"Xinyu_LED_Studio_V20.8.1_Engineering_Optimization.json")
}
...
function bootstrapApp(){installTooltips();
 try{
  dash();resize();bottom("ledmodels");summary();renderSceneTabs();renderGuides();projectHealth();
  startupCoreAudit();setFlow("scene");installActionGuard();installBusyFeedback();updateUXState();refreshActionGuards();updateContextRecommendation();setRuntimeState("2D核心已就緒","ok");setTimeout(buttonFunctionAudit,300);
